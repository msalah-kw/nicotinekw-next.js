  ...
  return (
    <nav className="mobile-nav-container" aria-label="شريط التنقل السفلي للهاتف">
      ...
      <Link
        ...
        aria-label="السلة"
      >
        <span id="mobile-cart-target" className={`mobile-nav-icon ${cartItemsCount > 0 ? "cart-has-items-pulse" : ""}`}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="22"
            height="22"
          ...
          {cartItemsCount > 0 && (
            <span id="mobile-cart-badge" className="mobile-nav-cart-badge" aria-label={`لديك ${cartItemsCount} منتجات في السلة`}>
              {cartItemsCount}
            </span>
          )}
        </span>
