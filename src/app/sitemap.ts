...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [productsRes, categoriesRes, pagesRes] = await Promise.all([
    fetchGraphQL(GET_ALL_PRODUCTS_QUERY, { first: 500 }, undefined, { revalidate: 3600 }).catch(err => {
      console.error("[Sitemap] Error fetching products:", err);
      return { data: { products: { nodes: [] } } };
    }),
    fetchGraphQL(GET_CATEGORIES_QUERY, {}, undefined, { revalidate: 3600 }).catch(err => {
      console.error("[Sitemap] Error fetching categories:", err);
      return { data: { productCategories: { nodes: [] } } };
    }),
    fetchGraphQL(GET_ALL_PAGES_QUERY, {}, undefined, { revalidate: 3600 }).catch(err => {
